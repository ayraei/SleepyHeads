...
public class ArrayManager {
	...
	private final int[] sums = new int[SVDPP_App.NUM_USERS];
	private final double[] Ns = new double[SVDPP_App.NUM_USERS];
	...
	public void initConstants() {
		...
		for (int i = 0; i < SVDPP_App.NUM_USERS; i++) {
			Ns[i] = Math.pow(this.getUserHistory_N(i).size(), -0.5);
			
			ArrayList<RateUnit> userHistory = this.getUserHistory_N(i);
			for (RateUnit ru : userHistory) {
				sums[i] += ru.getRating();
			}
		}
